...
export const messages: Record<string, string> = {
  ...
  'worker.submit_unconfirmed': '⚠️ Message submission auto-confirmation failed\nStage: input submission\nError code: submit_unconfirmed\nBotMux wrote the message and retried Enter, but could not confirm within {secs} seconds that {cliName} received it in {transcriptLabel}. Auto-confirmation failure does not mean the message did not run: it may still be running or may already have run.\nOpen the Web terminal and inspect the composer and run state. Resend only after confirming the message did not actually run.\nOriginal message: {preview}',
  'worker.submit_unconfirmed_zmx': '⚠️ Message submission auto-confirmation failed\nStage: input submission\nError code: submit_unconfirmed\nAfter {secs} seconds, BotMux still could not confirm that {cliName} received the message in {transcriptLabel}. Auto-confirmation failure does not mean the message did not run: it may still be running or may already have run.\nDo not resend blindly; run botmux list locally, enter the ZMX session, and inspect its composer and run state before deciding.\nOriginal message: {preview}',
